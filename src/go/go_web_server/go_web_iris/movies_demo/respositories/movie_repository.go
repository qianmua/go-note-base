...
这是可选的 （因为你也可以在 Service 内部使用），
不过在展示这个例子的时候我们需要创建一个 Repository ，
一个处理 “低级” 数据，可以直接访问 Movies 数据源的库。
保留一个 Repository ，它是一个 interface （接口） 因为可能会不同，
取决于您开发的应用程序的状态，
也就是说在 production 开发状态下它会使用某些真正的 SQL 查询数据或者其他一些您用于查询的数据。
